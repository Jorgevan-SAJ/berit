...
export function gerarPDFRelatorio({ titulo, subtitulo, lancamentos, totais, assinaturas, nomeArquivo, cnpj }) {
  const doc = new jsPDF()
  doc.setFontSize(11)
  doc.setTextColor(31, 58, 95)
  doc.text('Berit — Gestão simples para igrejas', 14, 14)

  // P3 — CNPJ institucional no cabeçalho, abaixo da denominação
  const cnpjFormatado = formatarCNPJ(cnpj)
  let yBase = 24
  if (cnpjFormatado) {
    doc.setFontSize(9)
    doc.setTextColor(90, 90, 90)
    doc.text(`CNPJ: ${cnpjFormatado}`, 14, 19)
    yBase = 26
  }

  doc.setFontSize(18)
  doc.setTextColor(31, 58, 95)
  doc.text(titulo, 14, yBase)
  doc.setFontSize(10)
  doc.setTextColor(90, 90, 90)
  doc.text(subtitulo, 14, yBase + 7)
...
export function gerarPDFContribuicoes({ ano, membros, nomeArquivo, cnpj }) {
  const doc = new jsPDF()
  doc.setFontSize(11)
  doc.setTextColor(31, 58, 95)
  doc.text('Berit — Gestão simples para igrejas', 14, 14)
  ...
  doc.setFontSize(18)
  doc.setTextColor(31, 58, 95)
  doc.text(`Relatório de Contribuições — ${ano}`, 14, yBase)
  doc.setFontSize(10)
  doc.setTextColor(90, 90, 90)
  doc.text('Tesouraria — comportamento dos membros em relação às contribuições', 14, yBase + 7)
